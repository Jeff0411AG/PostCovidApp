...
public interface IpreguntaRepository extends JpaRepository <Pregunta, Integer> {
    ...
    //
    @Query(value = "select d.id_doctor, d.name_doctor, d.last_name_doctor, count(d.name_doctor) preguntas from Pregunta p inner Join Doctor d on p.id_doctor = d.id_doctor group by d.id_doctor, d.name_doctor, d.last_name_doctor having count(d.name_doctor) <=2",nativeQuery = true)
    List<String[]>doctoresconmenospreguntas();
    ///
