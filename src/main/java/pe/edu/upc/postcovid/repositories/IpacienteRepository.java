...
public interface IpacienteRepository extends JpaRepository<Paciente,Integer>{
    ...
    @Query(value = "select * from paciente p where date_part('year', CURRENT_DATE) - date_part('year', p.birthday)>18 and p.numhistoria = '1'", nativeQuery = true )
    List<Paciente> buscarmayoredad();
    @Query (value = "select * from paciente p where date_part('year', CURRENT_DATE) - date_part('year', p.birthday)>18 and p.numhistoria >= '2'", nativeQuery = true)
    List<Paciente> segundodx();
}
